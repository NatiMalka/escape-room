...
type HackerChatProps = {
  hackerMessages: HackerMessage[];
  showHackerChat: boolean;
...
export default function Game() {
  ...
  // Add this near the beginning of the Game component before useEffect hooks
  const [showResults, setShowResults] = useState(false);
  const { players, leader, amILeader } = useSocket();
  ...
  return (
    <div className="min-h-screen flex flex-col bg-black text-gray-100">
      ...
      <Timer 
        ...
        isPaused={isPaused} 
        onTimeExpired={handleTimeExpired}
        onTimeChange={(time: number) => setTimeRemaining(time)} 
      />
                  ...
                  <div className="relative h-full p-4">
                    <div className="flex justify-between mb-4">
                      <div className="flex items-center">
                        <h2 className="text-green-400 font-mono text-xl mr-2">TOMAX Security</h2>
                        {isPaused ? (
                          <span className="bg-yellow-500 text-black px-2 py-1 rounded-md text-xs">PAUSED</span>
                        ) : (
                          <span className="bg-green-500 text-black px-2 py-1 rounded-md text-xs">ACTIVE</span>
                        )}
                      </div>
                    </div>
